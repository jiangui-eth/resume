...
const DECISIONS = [
  {
    ...
    explanation:
      'Search-critical pages render full HTML on the server before hydration.',
  },
  {
    title: 'Token-based theming',
    explanation:
      'Semantic tokens keep product variants aligned without duplicating component logic.',
  },
]
...
describe('technicalDecisions', () => {
  ...
  it('renders each decision explanation text', () => {
    renderWithIntl(<TechnicalDecisions decisions={DECISIONS} />)
    expect(
      screen.getByText(/render full HTML on the server/i),
    ).toBeInTheDocument()
    expect(
      screen.getByText(/semantic tokens keep product variants aligned/i),
    ).toBeInTheDocument()
  })
