...
async function hybridSearch(
  ...
  if (error)
    throw new Error(`Supabase hybrid_search error: ${error.message}`)
...
function buildSystemPrompt(context: FaqChunkRow[]): string {
  const contextBlock = context
    .map((c, i) => `[Source ${i + 1}] Q: ${c.question}\nA: ${c.answer}`)
    .join('\n\n')
...
function buildDemoAnswer(query: string): string {
  ...
  if (
    lq.includes('settlement')
    || lq.includes('how long')
    || lq.includes('time')
  ) {
  ...
  if (
    lq.includes('what is otc')
    || lq.includes('what\'s otc')
    || lq.includes('explain otc')
  ) {
